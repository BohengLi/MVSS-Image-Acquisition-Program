...
本文档用于说明 MVSS Capture 中独立的 DIC 图像采集功能。DIC 采集按钮位于主界面上方“定时拍照”参数区内，在“复位”按钮右侧，按钮文字为“DIC采集”；按钮右侧的 `DIC fps` 输入框用于设置本次 DIC 采集帧率。
...
下表为普通采集模式的仓库基线默认值，用于和 DIC 专用设置区分。软件会保存上一次界面状态；如果刚运行过 DIC 采集，`config.json` 顶层参数可能临时显示 DIC 参数，例如 `trigger_source=Cascade`、`record_fps=5.0`。DIC 功能的固定默认值以 `config.json` 的 `dic_capture` 段和程序内 `DIC_CAPTURE_CONFIG` 为准。
...
| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `trigger_source` | `Cascade` | 使用左 Master/右 Slave 级联硬触发 |
| `hardware_sync_master_line` | `Line2` | 左相机输出触发线 |
| `hardware_sync_master_line_source` | `ExposureActive` | Master 输出曝光有效信号 |
| `hardware_sync_slave_line` | `Line0` | 右相机接收触发线 |
...
| `record_fps` | `5.0` | DIC 录像和采集目标帧率，可通过界面 `DIC fps` 输入框修改 |
...
| `record_preview_fps` | `2.0` | 录像/采集过程中每 0.5s 刷新一次画面 |
...
3. 如需微秒级硬同步，加载“DIC 标准”预设或选择 `Cascade` 触发模式，并按默认拓扑接线：左相机 `Line2` 输出 `ExposureActive`，右相机 `Line0` 接收上升沿触发，两台相机共地。
4. 确认保存磁盘有足够空间。满幅双目 JPEG 序列加实时 MP4 会持续写入较多数据，建议使用 SSD。
5. 打开程序：
...
1. 在 `DIC fps` 输入框确认或输入目标采集帧率，然后点击“DIC采集”按钮。
2. 程序会自动应用 DIC 专用参数：
   - Cascade 级联硬触发；
   - Mono16；
   - 曝光 20000 us；
...
| 帧率 | 接近 `DIC fps` 输入值 | 若明显偏低，检查曝光、USB 带宽、磁盘速度 |
...
DIC 按钮默认使用 `Cascade` 级联硬触发：左相机作为 Master，软件只触发 Master，Master 通过 `Line2` 输出 `ExposureActive` 脉冲，右相机作为 Slave 通过 `Line0` 接收上升沿触发。若现场没有触发线，可临时切回 `Software`，但该模式存在主机调度抖动，不建议用于高精度位移重构。`Line0` 模式表示两台相机都等待同一个外部触发源，适用于已有独立触发器的接线方案。
